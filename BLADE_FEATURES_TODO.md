...
### 15. `<style scoped>` - 作用域樣式
- ✅ `<style scoped>` - 為組件提供樣式隔離
- ✅ 自動為 CSS 選擇器添加唯一屬性選擇器（如 `[data-v-xxxxx]`）
- ✅ 自動為 HTML 元素添加對應的 data 屬性
- ✅ 防止樣式洩漏到組件外部
- ✅ 支援類選擇器、ID 選擇器、元素選擇器
- ✅ 支援組合選擇器（後代、子代、相鄰）
- ✅ 支援偽類（`:hover`, `:focus` 等）
- ✅ 支援偽元素（`::before`, `::after` 等）
- ✅ 支援多個 `<style scoped>` 標籤
- ✅ 嵌套組件各自擁有獨立的 scopeId
- ✅ 與 slot 系統完全整合
- ✅ 與 @if/@foreach 等邏輯語法整合
- ✅ 保留 CSS 註釋和 @media queries
...
6. ✅ **`<style scoped>`** - 作用域樣式 (5 小時，24 個測試) - 2026-01-22
